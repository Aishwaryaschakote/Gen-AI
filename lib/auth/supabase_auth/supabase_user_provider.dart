...
class TryhereSupabaseUser extends BaseAuthUser {
  TryhereSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> tryhereSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TryhereSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
